...
const checkAuth = (team, lead, token) => {
...
const commandHandler = {
    lead: ({ team, lead, taggedLead, user_id, status, token }) => {
        return new Promise((resolve, reject) => {
            console.log(team)
            checkAuth(team, user_id, token).then(authed => {
    ...
    getLeads: ({ team }) => {
        return new Promise((resolve, reject) => {
            ...
            updateLeads(leads, team).then(() => resolve(leads.length ? `Lead${leads.length > 1 ? 's' : ''} on duty:\n\n${leads.map(lead => `\n${lead}`)}` : `No leads currently on duty`))
    ...
    check: ({ taggedLead, team }) => {
        return new Promise((resolve, reject) => {
            if (taggedLead) {
                updateLeads([taggedLead], team).then(() => resolve(`${taggedLead} is ${!team.leads[taggedLead] ? 'not' : ''} logged in`))
            } else {
