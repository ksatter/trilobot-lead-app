    ...
    getLeads: ({ team }) => {
        return new Promise((resolve, reject) => {
            ...
            updateLeads(leads, team)
                .then(() => resolve(leads.length ? `Senior LA${leads.length > 1 ? 's' : ''} on duty:\n\n${leads.map(lead => `\n${lead}`)}` : `No leads currently on duty`))
                .catch(() => reject())
        })
    },
        ...
        return new Promise((resolve, reject) => {
            checkAuth(team, user_id).then(authed => {
                if (authed) {
                    team.leads = {};
                    team.las = {};
                    resolve(`Senior LA List Cleared`)
                } else {
...
//Clears leads list using data from cron-job.org. Runs daily at 1:00AM CST
const cronClear = (data) => {
    return new Promise ((resolve, reject) => {

        Object.keys(workspaces).forEach((team) => {
            commandHandler.clear({team: workspaces[team], user_id: data[team]})
                .then(message => resolve("done"))
                .catch(message => reject("error"))
        })
    })
}
module.exports = {leadMessage, cronClear, checkAuth}